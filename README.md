...
A safe and effective method for deploying using the Open Zeppelin Ownable library is to use _transferOwnership(wallet_address) in the constructor setup, make sure to use the same wallet address in the constructor for all chains in your initial deployment. If there is a need to change the owner address again on chain, then use the transferOwnership function again on chain to preserve the single contract address. This is shown to effectively transfer ownership from the factory contract address to the set contract address. 
[More information here](https://docs.openzeppelin.com/contracts/2.x/api/ownership#Ownable-_transferOwnership-address-)
